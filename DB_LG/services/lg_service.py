...
async def chat_with_llm(request: CaseChatRequest) -> AsyncIterator[str]:
    ...
    根据这些信息从数据库获取 case info
        ...
        async for chunk in get_chat_response_stream_langchain(messages, system_prompt=__system_prompt, model_name="r1",if_r1=True):
            yield chunk

        # 往https://nlp-demo.szmckj.cn/forward_ai 发送请求。请求体和get_chat_response_stream_langchain的参数一样。
...
async def generate_reply_by_llm(kb_content: List[dict], chat_history: List[dict], user_input: str, if_r1: bool) -> AsyncIterator[str]:
    ...
    try:
        ...
        async for chunk in get_chat_response_stream_langchain(messages, model_name=__model_name, system_prompt=__system_prompt,if_r1=if_r1):
            yield chunk
